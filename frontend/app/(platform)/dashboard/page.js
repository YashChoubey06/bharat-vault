...
import { getDashboardStats as getMockDashboardStats, calculateDilrmpProgress } from "@/services/mock/dashboard";
import { getVerificationCases as getMockVerificationCases } from "@/services/mock/verification";
import { getAuditLogs as getMockAuditLogs } from "@/services/mock/audit";
...
function DashboardContent() {
  ...
  const [data, setData] = useState({
    ...
    dilrmpProgress: calculateDilrmpProgress({}),
  });
  ...
  useEffect(() => {
    async function loadScopedData() {
      try {
        ...
        let statsData, verificationData, auditData;

        try {
          [statsData, verificationData, auditData] = await Promise.all([
            getDashboardStats(scope),
            getVerificationCases(scope),
            getAuditLogs(scope),
          ]);
        } catch (apiErr) {
          console.warn("Backend API call failed, falling back to mock scoped data:", apiErr);
          [statsData, verificationData, auditData] = await Promise.all([
            getMockDashboardStats(scope),
            getMockVerificationCases(scope),
            getMockAuditLogs(scope),
          ]);
        }

        if (!statsData) {
          statsData = await getMockDashboardStats(scope);
        }
        ...
        if (statsData) {
          setData({
            ...
            recordHealth: statsData.recordHealth ?? 93,
            processedRecords: statsData.processedRecords ?? 0,
            processingToday: statsData.processingToday ?? 0,
            averageProcessingTime: statsData.averageProcessingTime || "3.8s",
            dilrmpProgress: statsData.dilrmpProgress || statsData.dilrmp_progress || calculateDilrmpProgress(scope),
          });
        }

        if (!verificationData || verificationData.length === 0) {
          verificationData = await getMockVerificationCases(scope);
        }

        setCases(verificationData || []);

        if (!auditData || auditData.length === 0) {
          auditData = await getMockAuditLogs(scope);
        }
