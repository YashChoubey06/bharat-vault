...
import { LAND_DATASETS } from '@/components/studio/mockStudioData';
...
export default function StudioPage() {
  ...
  const [selectedDatasetId, setSelectedDatasetId] = useState('svamitwa-drone');
  const [activeBuildingId, setActiveBuildingId] = useState('SVAMITVA-KA-BLR-0042-02');
  ...
  const currentDataset = LAND_DATASETS.find((d) => d.id === selectedDatasetId) || LAND_DATASETS[0];
    ...
    <div className="relative w-full h-screen bg-slate-100 flex flex-col overflow-hidden font-sans select-none">
      ...
      <header className="w-full bg-white border-b border-slate-200 px-4 py-2.5 flex items-center justify-between z-30 shrink-0 shadow-xs">
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            {LAND_DATASETS.map((ds) => (
              <option key={ds.id} value={ds.id}>
                {ds.name} ({ds.bldgsCount} bldgs)
              </option>
            ))}
          </select>
        </div>
        ...
        {activeFloor && (
          <UnitDetailsDrawer
            ...
            onOpenReachCitizenModal={(floor) => {
              alert(`Field Navigation & QR Scanner dispatched for ULPIN: ${floor.ulpin}`);
            }}
          />
        )}
