...
import { getParcelById as getMockParcelById } from "@/services/mock/parcels";
import { getEvidenceByParcel as getMockEvidenceByParcel } from "@/services/mock/validation";
import { getDocumentsByParcel as getMockDocumentsByParcel } from "@/services/mock/documents";
  ...
  useEffect(() => {
    async function loadGraphData() {
      try {
        ...
        try {
          parcelData = await getParcelById(parcelId);
        } catch (e) {
          console.warn("API getParcelById failed, falling back to mock:", e);
          parcelData = await getMockParcelById(parcelId);
        }
        ...
        try {
          evidenceData = await getEvidenceByParcel(parcelId);
        } catch (e) {
          console.warn("API getEvidenceByParcel failed, falling back to mock:", e);
          evidenceData = await getMockEvidenceByParcel(parcelId);
        }

        try {
          documentData = await getDocumentsByParcel(parcelId);
        } catch (e) {
          console.warn("API getDocumentsByParcel failed, falling back to mock:", e);
          documentData = await getMockDocumentsByParcel(parcelId);
        }
...
function buildGraphData(parcel, evidence, documents) {
  ...
  if (parcel.gis) {
    addNode({
      ...
      metadata: {
        area: parcel.gis.area !== undefined ? `${parcel.gis.area} ha` : "2.20 ha",
