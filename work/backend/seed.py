    ...
    with db.transaction() as con:
        ...
        if existing:
            ...
            for index, parcel in enumerate(existing):
                validation = db.get(con, 'validation', parcel['id'])
                if (validation or {}).get('engineVersion') != 'evidence-resolution-v2' and not parcel.get('archivedAt'):
                    if validation:
                        db.put(con, 'legacy_validation', dict(id=parcel['id'], snapshot=validation, preservedAt=db.now()))
