...
class LocalMVPTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.original_data=db.DATA
        db.DATA=Path(cls.temp.name)
        target, cls.cleanup_database = test_database(db.DATA)
        cls.env=patch.dict(os.environ,{'BHARAT_DATABASE_URL':target,'BHARAT_SOURCE_MODE':'disabled'})
        cls.env.start()
    ...
    def test_end_to_end_real_ocr(self):
        ...
        self.assertTrue(any(c['type']=='AREA_MISMATCH' for c in conflicts),conflicts)
        ...
        self.assertEqual(self.client.get('/api/v1/parcels/PRC-003').json()['recordStatus'],'CONDITIONAL_REVIEWED')
        self.assertEqual(decision.json()['externalVerification'],'NOT_VERIFIED')
