...
def health():
    try:
        with db.transaction() as con: con.execute('SELECT 1').first()
    except Exception:
        raise HTTPException(503, 'Database is unavailable.') from None
    return dict(status='ok', storage=db.get_engine().dialect.name, ocr=ocr.health(), mode='evidence-resolution', sourceMode=os.getenv('BHARAT_SOURCE_MODE','disabled'))
...
def login(body: Login, request: Request, response: Response):
    ...
    with db.transaction() as con:
        ...
        if not valid:
            ...
            con.execute('INSERT INTO login_attempts VALUES(?,?,?) ON CONFLICT(identity) DO UPDATE SET failures=excluded.failures,reset_at=excluded.reset_at', (identity,failures,time.time()+900))
        else:
...
def decide(key: str,body: Decision,user=User):
    ...
    with db.transaction() as con:
        ...
        validation=db.get(con,'validation',p['id']) or {}
        checks=validation.get('checks',[])
        if status=='VERIFIED' and (any(item.get('status')=='OPEN' for item in validation.get('conflicts',[])) or any(item['status'] in ('REVIEW_REQUIRED','HIGH_RISK','CRITICAL','HUMAN_REVIEW_REQUIRED') for item in checks)):
        ...
        resolution=validation.get('externalResolution',{})
        if status=='VERIFIED' and (resolution.get('externalVerification')!='CORROBORATED' or resolution.get('unresolvedEvidenceCount',1)):
            status='CONDITIONAL_REVIEWED'
        c.update(status=status,notes=body.notes.strip(),decision=body.decision,reviewedBy=user['id'],reviewedAt=db.now())
        p.update(recordStatus='REVIEW_REQUIRED' if status=='PENDING_REVIEW' else status,
                 status='REVIEW_REQUIRED' if status=='PENDING_REVIEW' else status,updatedAt=db.now())
        if status in ('VERIFIED','CONDITIONAL_REVIEWED'):
            ...
            for item in validation.get('conflicts',[]):
                if item.get('status')!='RESOLVED_BY_OFFICER': continue
                accepted=item['resolution']
                if item['field']=='current_holder':
                    p['currentRecordedOwner']=accepted['acceptedValue']
                    p['owner']={'name':accepted['acceptedValue']}
                elif item['field']=='area':
                    p['recordedArea']=accepted['normalizedValue']
        c['externalVerification']=resolution.get('externalVerification','NOT_VERIFIED')
        c['decisionBasis']='OFFICER_REVIEW_WITH_EXTERNAL_EVIDENCE' if status=='VERIFIED' else 'OFFICER_DOCUMENTARY_REVIEW'
        c['runId']=validation.get('runId')
        validation['verification']=dict(required=status=='PENDING_REVIEW',caseId=key,status=status,
            reviewedBy=user['id'],reviewedAt=c['reviewedAt'],externalVerification=c['externalVerification'])
        db.put(con,'validation',validation)
        db.put(con,'case',c)
        db.put(con,'parcel',p)
        db.audit(con,user['id'],'CASE_DECISION_RECORDED',p['id'],'Officer decision saved.',
            {'caseId':key,'decision':status,'notes':body.notes.strip(),'runId':validation.get('runId'),
             'externalVerification':c['externalVerification'],'decisionBasis':c['decisionBasis'],
             'resolvedConflicts':[item['id'] for item in validation.get('conflicts',[]) if item.get('status')=='RESOLVED_BY_OFFICER']})
        return joined_case(con,c)
...
def assistant(body: Question,user=User):
    ...
    with db.transaction() as con:
        ...
        if any(term in body.question.casefold() for term in ('why','conflict','risk','source status','verification','unavailable')):
            validation=db.get(con,'validation',body.parcelId) or {}
            checks=validation.get('checks',[])
            relevant=[item for item in checks if item['status']!='MATCH'] or checks
            explanation='\n'.join(item.get('explanation',item.get('message','')) for item in relevant)
            references=[dict(id=item.get('id',item['check']),parcelId=body.parcelId,sourceType='VALIDATION',documentName='Evidence resolution '+str(validation.get('runId','')),
                page=None,value=item.get('message',''),reviewStatus=item['status'],runId=validation.get('runId'),
                sourceRecordIds=item.get('sourceReferences',[])) for item in relevant]
            return dict(id=db.uid('ANS'),parcelId=body.parcelId,question=body.question,
                answer=explanation or 'Run evidence resolution to obtain a source-backed explanation.',sources=references,
                generatedAt=db.now(),mode='structured-evidence-explanation')
...
from .resolution.api import router as resolution_router
